...
if st.session_state['scraping_state'] == 'waiting':
    ...
    with st.spinner('Scraping in progress...'):
        ...
        if st.session_state['attended_mode'] and driver is not None:
            ...
            # Determine URLs to scrape (pagination or original)
            if pagination_info and pagination_info["page_urls"]:
                all_page_urls = pagination_info["page_urls"]
                st.success(f"Detected {len(all_page_urls)} pagination pages.")
            else:
                all_page_urls = st.session_state['urls']
                st.info(f"No pagination detected, scraping original URL(s).")

            # Optional: Allow user to limit how many pages they want to scrape
            max_pages = st.sidebar.number_input(
                "Max Pages to Scrape", min_value=1, max_value=len(all_page_urls), value=min(5, len(all_page_urls))
            )

            all_page_urls = all_page_urls[:max_pages]  # limit pages

            # Progress bar
            progress_bar = st.progress(0)

            for i, url in enumerate(all_page_urls, start=1):
                st.write(f"Scraping page {i}: {url}")
                ...
                try:
                    ...
                    # Update progress
                    progress_bar.progress(i / len(all_page_urls))

                    # Random small sleep to avoid hammering server
                    time.sleep(random.uniform(1.5, 3.0))

                except Exception as e:
                    st.error(f"Error scraping {url}: {e}")

            # After loop, finish progress bar
